...
export function registerCommentTools(
	...
	options?: { isRemoteMode?: boolean; readOnly?: boolean },
): void {
	...
	// -----------------------------------------------------------------------
	// Tool: figma_post_comment (write — skipped when readOnly)
	// -----------------------------------------------------------------------
	if (!readOnly) {
		server.tool(
			"figma_post_comment",
							...
							content: [
								{
									...
									text: JSON.stringify({
										error: "no_file_url",
										message:
											"No Figma file URL available. Pass the fileUrl parameter, call figma_navigate (CDP mode), or ensure the Sparrow Bridge Plugin is connected (WebSocket mode).",
									}),
								},
							],
							isError: true,
	...
	// -----------------------------------------------------------------------
	// Tool: figma_delete_comment (write — skipped when readOnly)
	// -----------------------------------------------------------------------
	if (!readOnly) {
		server.tool(
